...
# output contig assembly positions
print("output contig positions")
seqnames<- sapply(strsplit(posteriors$bpos, ":"), "[", 1)
start<- as.numeric(sapply(strsplit(posteriors$bpos, ":"), "[", 2))
end<- start
contig.positions<- makeGRangesFromDataFrame(data.frame(seqnames, start, end))
contig.positions<- flank(contig.positions, width = 100, start = T, both = T)
export.bed(contig.positions, filename.contigs)
