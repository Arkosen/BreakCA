...
then seqtk subseq $reverse_reads $output/read.Ids.txt  > $output/r2.fastq
 ...
 # run SPAdes
if (( $pe > 0 ))
then spades.py -t $threads -k 21,33 --careful -o $output/spades -1 $output/r1.fastq -2 $output/r2.fastq
else spades.py -t $threads -k 21,33 --careful -o $output/spades -s $output/unpaired.fastq
fi
