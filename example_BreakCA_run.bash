...
# Train the Random Forest classifier. Note this is a time-consuming step but it only needs to be run once:
$R_PATH --vanilla misc/build_randomForest.R training.txt model.rda

# Predict indels using the trained Random Forest:
$R_PATH --vanilla bin/make_predictions.R classifier_input.tsv model_paired_end_no_GATK.rda prediction.txt 


# .bam files can be visualized on IGV (https://software.broadinstitute.org/software/igv/)
