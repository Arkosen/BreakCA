...
# is window supported by local mapped reads contig assembly
print("get contigs and determine windows supported by contigs")
contigs<- makeGRangesFromDataFrame(read.table(contigs,header=T))
overlaps<- findOverlaps(contigs, win)
contigs.supported.windows<- mcols(win[subjectHits(overlaps)])$windows

# add contig support
ranked$cs[ranked$windows %in% contigs.supported.windows]<- 1
ranked$cs[is.na(ranked$cs)]<-0
